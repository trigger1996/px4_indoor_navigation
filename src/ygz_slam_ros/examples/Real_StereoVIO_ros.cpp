...
void FetchImageCallback(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    /// 增加
    if (imRightRect.type() == CV_8UC3) {
        cvtColor(imLeftRect,  imLeftRect,  CV_BGR2GRAY);
        cvtColor(imRightRect, imRightRect, CV_BGR2GRAY);
    }
    ...
    EstimatedPose.pose.position.y = DroneFrameSE3_t[0] * cos(pixhawk_heading_rad) + DroneFrameSE3_t[1] * sin(pixhawk_heading_rad);
...
int main(int argc, char **argv) {
    ...
    if (argc > 2)
        google::InitGoogleLogging(argv[2]);
    else
